            ...
            VStack(alignment: .center, spacing: 32) {
                VStack(alignment: .center, spacing: 8) {
                    Text("emux")
                        .bold()
                        .font(.title)
                    Text("A project-scoped terminal built on top of\nGhostty's libghostty terminal core.")
                ...
                VStack(alignment: .leading, spacing: 6) {
                    Text("Acknowledgements")
                        .font(.headline)
                    Text("emux is a fork of Ghostty's macOS app shell, with libghostty consumed as a vendored library.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                        .fixedSize(horizontal: false, vertical: true)
                    Text("Ghostty © Mitchell Hashimoto and Ghostty contributors. MIT licensed.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                        .textSelection(.enabled)
                        .fixedSize(horizontal: false, vertical: true)
                    if let url = URL(string: "https://github.com/ghostty-org/ghostty") {
                        Link("github.com/ghostty-org/ghostty", destination: url)
                            .font(.caption)
                    }
                }
